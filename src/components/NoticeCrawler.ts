...
import { SiteScript } from "@src/interfaces";
...
class NoticeCrawler {
  ...
  craweling(site: SiteScript) {


  }

  async getNoticeList(site: SiteScript) {
    try {
      ...
      // 공지사항 목록 가져오기
      const notice_list = await this.cralwer.evaluate(`return getData()`);
