...
const { ADMIN_NOT_FOUND, NICKNAME_DUPLICATED, LOGIN_ID_DUPLICATED, DB_ERROR } = errors;
@Injectable()
export class AdminService {
  ...
  async create(adminCreateDto: AdminCreateDto): Promise<Admin>{
    const foundAdmins = await this.adminRepository.find({ where: [ { loginId: adminCreateDto.loginId }, { nickname: adminCreateDto.nickname } ] });
    foundAdmins.forEach(foundAdmin => {
      if (foundAdmin.loginId === adminCreateDto.loginId) throw LOGIN_ID_DUPLICATED;
    });
    // DB조회를 줄이기 위해 이렇게 했습니다.
    foundAdmins.forEach(foundAdmin => {
      if (foundAdmin.nickname === adminCreateDto.nickname) throw NICKNAME_DUPLICATED;
    });
